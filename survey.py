...
# Команда /survey для начала анкеты
...
async def start_survey(message: Message, state: FSMContext):
    ...
    if db.user_exists(user_id):
        ...
        await message.answer(
            "👋 Давайте заполним анкету для записи на курс!\n\n"
            "Введите ваше имя:",
...
async def process_first_name(message: Message, state: FSMContext):
    if message.text.lower() == 'отмена':
        await state.clear()
        await message.answer("❌ Анкета отменена", reply_markup=main_keyboard)
        return
...
async def process_course(message: Message, state: FSMContext):
    ...
    result_text = (
        ...
        f"<b>Администратор свяжется с вами в течение 24 часов для подтверждения записи.</b>\n\n"
